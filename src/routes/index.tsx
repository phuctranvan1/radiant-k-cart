...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Discover premium K-beauty essentials handpicked from Seoul. Skincare, makeup, sun care and curated sets." },
    ],
  }),
  component: Index,
});
...
type Product = {
  ...
  rating: number | null; is_new: boolean | null;
};
...
function Index() {
  ...
  useEffect(() => {
    supabase.from("products").select("id,name,slug,brand,price,sale_price,image_url,rating,is_new")
      .eq("featured", true).limit(8).then(({ data }) => setFeatured(data ?? []));
    supabase.from("products").select("id,name,slug,brand,price,sale_price,image_url,rating,is_new")
      .eq("is_new", true).limit(4).then(({ data }) => setNewest(data ?? []));
    ...
    <div>
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0">
          <img src={heroImg} alt="Luxury K-beauty serums" className="w-full h-full object-cover opacity-60" width={1920} height={1080} />
        ...
        <div className="relative container mx-auto px-4 py-32 md:py-44 max-w-3xl">
          ...
          <div className="flex flex-wrap gap-3">
            <Link to="/products">
              <Button size="lg" className="bg-gradient-gold text-primary-foreground hover:opacity-90 px-8">Shop the Edit</Button>
            </Link>
            <Link to="/about">
              <Button size="lg" variant="outline" className="border-gold text-gold hover:bg-gold/10">Our Story</Button>
            </Link>
          </div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {categories.map((c) => (
            <Link key={c.id} to="/categories/$slug" params={{ slug: c.slug }}
              ...
              <p className="font-display text-lg group-hover:text-gold transition-colors">{c.name}</p>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-20">
        <div className="flex items-end justify-between mb-12">
          ...
          <Link to="/products" className="hidden md:block text-sm text-gold hover:underline">View all →</Link>
        </div>
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="luxe-card rounded-2xl p-12 text-center bg-gradient-to-br from-card to-secondary border-gold">
          ...
          <p className="text-muted-foreground mb-6">Use code <span className="text-gold font-mono font-bold tracking-widest">GLOW20</span> at checkout</p>
          <Link to="/products"><Button size="lg" className="bg-gradient-gold text-primary-foreground">Shop now</Button></Link>
        </div>
      </section>
