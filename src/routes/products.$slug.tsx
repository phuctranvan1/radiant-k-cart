...
export const Route = createFileRoute("/products/$slug")({
  ...
  notFoundComponent: () => (
    <div className="container py-20 text-center">
      ...
      <Link to="/products" className="text-gold underline">Back to all products</Link>
    </div>
  ),
});
...
function ProductDetail() {
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <Link to="/products" className="text-sm text-muted-foreground hover:text-gold">← Back to products</Link>
      ...
      <div className="grid md:grid-cols-2 gap-12 mt-6">
        ...
        <div>
          {p.brand && <p className="text-xs tracking-widest text-muted-foreground uppercase mb-2">{p.brand}</p>}
          ...
          <div className="flex items-center gap-4 mb-6">
            <div className="flex items-center border border-border rounded-md">
              <button onClick={() => setQty(Math.max(1, qty - 1))} className="px-3 py-2 hover:text-gold"><Minus size={16} /></button>
              <span className="px-4 py-2 min-w-[3rem] text-center">{qty}</span>
              <button onClick={() => setQty(qty + 1)} className="px-3 py-2 hover:text-gold"><Plus size={16} /></button>
            </div>
            <Button size="lg" className="flex-1 bg-gradient-gold text-primary-foreground hover:opacity-90"
              onClick={() => addItem(p.id, qty)} disabled={p.stock === 0}>
              {p.stock === 0 ? "Out of stock" : "Add to bag"}
            </Button>
            <Button size="lg" variant="outline" className="border-gold text-gold"><Heart size={18} /></Button>
          </div>

          <div className="grid grid-cols-2 gap-3 mb-8 text-xs">
            <div className="flex items-center gap-2 text-muted-foreground"><Truck size={14} style={{ color: "var(--gold)" }}/> Free shipping over $80</div>
            <div className="flex items-center gap-2 text-muted-foreground"><Shield size={14} style={{ color: "var(--gold)" }}/> Authentic guaranteed</div>
          </div>
