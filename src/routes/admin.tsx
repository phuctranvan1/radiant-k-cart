...
function AdminPage() {
  ...
  if (!isAdmin) return <div className="container py-20 text-center text-muted-foreground">Checking permissions…</div>;
      ...
      <div className="flex justify-between mb-4">
        ...
        <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) setEditing(null); }}>
          <DialogTrigger asChild>
            <Button className="bg-gradient-gold text-primary-foreground"><Plus size={16} /> New</Button>
          </DialogTrigger>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle>{editing ? "Edit" : "New"} product</DialogTitle></DialogHeader>
            <form onSubmit={save} className="grid grid-cols-2 gap-3">
              <div className="col-span-2"><Label>Name</Label><Input name="name" required defaultValue={editing?.name} /></div>
              ...
              <div className="col-span-2"><Label>Description</Label><Textarea name="description" rows={3} defaultValue={editing?.description} /></div>
              ...
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" name="is_new" defaultChecked={editing?.is_new} /> New</label>
              <Button type="submit" className="col-span-2 bg-gradient-gold text-primary-foreground">Save</Button>
            </form>
          </DialogContent>
        </Dialog>
      </div>
        ...
        {products.map((p) => (
          <div key={p.id} className="flex items-center gap-4 p-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground">${p.price} · stock {p.stock} · {p.categories?.name}</p>
            </div>
            <Button size="sm" variant="ghost" onClick={() => { setEditing(p); setOpen(true); }}><Edit size={14} /></Button>
            <Button size="sm" variant="ghost" onClick={() => del(p.id)} className="text-destructive"><Trash2 size={14} /></Button>
          </div>
        ))}
      </div>
    </div>
...
function OrdersAdmin() {
  ...
  return (
    <div className="mt-6 luxe-card rounded-xl divide-y divide-border">
      {orders.length === 0 ? <p className="p-6 text-muted-foreground">No orders yet.</p> : orders.map((o) => (
...
function PromosAdmin() {
  ...
  const create = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (error) return toast.error(error.message);
    toast.success("Promo created"); (e.target as HTMLFormElement).reset(); refresh();
  };
  const del = async (id: string) => { await supabase.from("promo_codes").delete().eq("id", id); refresh(); };
  return (
    <div className="mt-6 grid lg:grid-cols-[1fr_300px] gap-6">
      ...
      <form onSubmit={create} className="luxe-card rounded-xl p-4 space-y-3 h-fit">
        ...
        <div><Label>Min order ($)</Label><Input name="min_order" type="number" step="0.01" defaultValue="0" /></div>
        <Button type="submit" className="w-full bg-gradient-gold text-primary-foreground">Create</Button>
      </form>
    </div>
  );
}
...
function TicketsAdmin() {
  ...
  return (
    <div className="mt-6 luxe-card rounded-xl divide-y divide-border">
      {tickets.length === 0 ? <p className="p-6 text-muted-foreground">No support tickets.</p> : tickets.map((t) => (
        <div key={t.id} className="p-4">
          <div className="flex justify-between mb-1">
            <p className="font-medium">{t.subject}</p>
            <span className="text-xs text-muted-foreground">{new Date(t.created_at).toLocaleDateString()}</span>
          </div>
          <p className="text-xs text-muted-foreground mb-2">{t.name} · {t.email}</p>
